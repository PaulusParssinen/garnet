...
public class RespAofTests
{
    ...
    public void AofUpsertObjectStoreRecoverTest()
    {
        ...
        using (var redis = ConnectionMultiplexer.Connect(TestUtils.GetConfig()))
        {
            ...
            RedisValue[] result = db.ListRange(key2);
            Assert.AreEqual(origList, result);
        }
    }
    ...
    public void AofUpsertCustomObjectRecoverTest()
    {
        ...
        string mainKey1 = "key1";
        string subKey = "subKey";
        string subKeyValue = "subKeyValue";
        string mainKey2 = "key2";
        using (var redis = ConnectionMultiplexer.Connect(TestUtils.GetConfig(allowAdmin: true)))
        {
            IDatabase db = redis.GetDatabase(0);

            db.Execute("MYDICTSET", mainKey1, subKey, subKeyValue);
            RedisResult retValue = db.Execute("MYDICTGET", mainKey1, subKey);
            Assert.AreEqual(subKeyValue, (string)retValue);

            bool rb = db.KeyRename(mainKey1, mainKey2);
            Assert.IsTrue(rb);
        ...
        using (var redis = ConnectionMultiplexer.Connect(TestUtils.GetConfig()))
        {
            ...
            RedisResult retValue = db.Execute("MYDICTGET", mainKey2, subKey);
            Assert.AreEqual(subKeyValue, (string)retValue);
        }
    }
    ...
    {
        string key = "AofListObjectStoreRecoverTest";
        ...
        using (var redis = ConnectionMultiplexer.Connect(TestUtils.GetConfig()))
        {
            ...
            long count = db.ListLeftPush(key, ldata);
            Assert.AreEqual(4, count);
